...
#define PC104_IRQ_GPIO          192
#define TS42XX_PC104IO8         0x10000400
#define TS42XX_PC104IO16        0x30000400
...
static int ax_init_dev(struct net_device *dev, int first_init)
{
	...
	if (first_init && ax->plat->flags & AXFLG_HAS_EEPROM) {
	   ...
	   macaddr_4200[5]++;
	   if(0x00 == macaddr_4200[5]) macaddr_4200[4]++;
	   if(0x00 == macaddr_4200[4]) macaddr_4200[3]++;
	   memcpy(dev->dev_addr, macaddr_4200, sizeof(macaddr_4200));	   	   
	}
#endif
	...
	if (first_init) {
		printk("AX88796b: %dbit, irq %d (PC/104 IRQ %d), %lx, MAC: ",
		       ei_status.word16 ? 16:8, dev->irq, dev->irq - (PC104_IRQ_GPIO - 5), 
		       dev->base_addr);
...
static int ax_probe(struct platform_device *pdev)
{
   ...
   dev->base_addr = mem;
	...
	if(dev->base_addr == 0 ) {
	   unsigned char *cpld;
	   cpld = (unsigned char *)request_mem_region(TS42XX_PC104IO8, 512, 
	      pdev->name);
	   if (cpld == NULL) {
	      dev_err(&pdev->dev, "cannot reserve memory for cpld regs," 
	         " ignoring jumpers\n");	      		
	   } else {
	         unsigned long cpldbase;
	         cpldbase = (unsigned long)ioremap(TS42XX_PC104IO8, 0x10);
	         if (cpldbase == 0) {
	            dev_err(&pdev->dev, "cannot ioremap regs for cpld, "
	               "ignoring jumpers\n");
	         } else {
	            if ( (inb(cpldbase|0x100)&0xF) == 0x5 && // unique ID == 5
	               (inb(cpldbase|0x104)&0xF) == 0x1 ) {  // ...and version = 1
	               dev->base_addr = 0x200;
	               if ((inb(cpldbase|0x108)&0xE) == 0x8) 
	                  dev->irq = 7;
	               else if ((inb(cpldbase|0x108)&0xE) == 0x4) 
	                  dev->irq = 6;
	               else if ((inb(cpldbase|0x108)&0xE) == 0x2) 
	                  dev->irq = 5;		  		  
	            } else if ( (inb(cpldbase|0x110)&0xF) == 0x5 && 
	               (inb(cpldbase|0x114)&0xF) == 0x1 ) {
	               dev->base_addr = 0x240;
	               if ((inb(cpldbase|0x118)&0xE) == 0x8) 
	                  dev->irq = 7;
	               else if ((inb(cpldbase|0x118)&0xE) == 0x4) 
	                  dev->irq = 6;
	               else if ((inb(cpldbase|0x118)&0xE) == 0x2) 
	                  dev->irq = 5;		  
	            } else if ( (inb(cpldbase|0x120)&0xF) == 0x5 && 
	               (inb(cpldbase|0x124)&0xF) == 0x1 ) {
	               dev->base_addr = 0x300;
	               if ((inb(cpldbase|0x128)&0xE) == 0x8) 
	                  dev->irq = 7;
	               else if ((inb(cpldbase|0x128)&0xE) == 0x4) 
	                  dev->irq = 6;
	               else if ((inb(cpldbase|0x128)&0xE) == 0x2) 
	                  dev->irq = 5;		  		  
	            } else if ( (inb(cpldbase|0x130)&0xF) == 0x5 && 
	                 (inb(cpldbase|0x134)&0xF) == 0x1 ) {
	               dev->base_addr = 0x340;
	               if ((inb(cpldbase|0x138)&0xE) == 0x8) 
	                  dev->irq = 7;
	               else if ((inb(cpldbase|0x138)&0xE) == 0x4) 
	                  dev->irq = 6;
	               else if ((inb(cpldbase|0x138)&0xE) == 0x2) 
	                  dev->irq = 5;		  		  
	            } 
	         iounmap((void*)cpldbase);
		   }
		  release_mem_region(TS42XX_PC104IO8, 512); 
		}
	}
#endif
	
   dev->irq += (PC104_IRQ_GPIO - 5);
	size = 0x20;

	ei_status.reg_offset = ax->reg_offsets;
	for (ret = 0; ret < 0x20; ret++)
		ax->reg_offsets[ret] = ret;
	
	ax->map2 = NULL;
	
	ax->mem = request_mem_region(TS42XX_PC104IO16 + dev->base_addr, size, 
	   pdev->name);
	if (ax->mem == NULL) {
	...
	ei_status.mem = ioremap(TS42XX_PC104IO16 + dev->base_addr, size);
	dev->base_addr = (unsigned long)ei_status.mem;

	if (ei_status.mem == NULL) {
		dev_err(&pdev->dev, "Cannot ioremap area (%08llx,%08llx)\n",
			(unsigned long long)TS42XX_PC104IO16 + dev->base_addr,
			(unsigned long long)TS42XX_PC104IO16 + dev->base_addr + size);
 ...
 exit_mem:
	...
	if (dev->base_addr != 0) {
	  dev_err(&pdev->dev, 
	     "AX88796B: No AX88796B card found at memory = %#lx, error = %d\n", 
	     dev->base_addr, ret);
	}
	free_netdev(dev);
...
MODULE_DESCRIPTION("AX88796b 10/100 Ethernet platform driver");
