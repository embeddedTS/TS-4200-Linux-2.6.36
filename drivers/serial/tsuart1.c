...
static void tsuart_handleRx(struct tsuart_port *port,int data) {
  ...
  int flag = 0;
  struct uart_state *state = port->u.state;
  struct tty_struct *tty = state->port.tty;

  //printk("%02X ",ch);
  if (port->debugFlags & DEBUG_DATA) {
    unsigned int ttyinf = state ? (tty ? 3:1) : 0;
...
static void tsuart_try_handleTx(struct tsuart_port *port) {
  ...
  if (port->u.flags & ASYNC_CTS_FLOW) {
	...
	if (port->debugFlags & DEBUG_INTERRUPT) {
	  if (tty->hw_stopped) printk("CTS start\n");
	}
	tty->hw_stopped = 0;
      } else {
	if (port->debugFlags & DEBUG_INTERRUPT) {
	  if (!tty->hw_stopped) printk("CTS stop\n");
	}
	tty->hw_stopped = 1;
	if (!timer_added) {
	  timer_added = 1;
...
static int tsuart_request_port(struct uart_port *port) {
  ...
  if (port->iotype == SERIAL_IO_MEM) {
    if (port->membase == 0) {
      port->membase = (void *) ioremap_nocache(port->mapbase & 0xFFFFF000, 4096)
	+ (port->mapbase & 0xFFF);
