...
#define FPGA_SYSCON_BASE   0x30000000
#define NR_UARTS  2
...
static int __init ts4200_isa16550_init(void)
{
	int line;
	...
	syscon = (unsigned short *)ioremap(FPGA_SYSCON_BASE, 4096);
	if (syscon == 0) {
	   printk("ts4200_isa16550: could not get syscon register\n");
	   return -ENOMEM;    
	}

	
	syscon[0x20/2] = 0x181;     // enable pc104 
   ...
   iounmap(syscon);
	...
	req[0].regshift = 0;
	req[0].irq = 187 + uart_irq[0];
	req[0].mapbase = 0x10000400 + uart_adr[0];
	...
	req[1].regshift = 0;
	req[1].irq = 187 + uart_irq[1];
	req[1].mapbase = 0x10000400 + uart_adr[1];
...
static void __exit ts4200_isa16550_exit(void)
{

}


module_init(ts4200_isa16550_init);
module_exit(ts4200_isa16550_exit);
